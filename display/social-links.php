...
function nightingale_companion_widgetise_header_enqueue() {
	wp_enqueue_script( 'socialfloating', plugin_dir_url( __FILE__ ) . 'js/jquery.socialfloating.js', array( 'jquery' ), '0.1.0' );
	wp_enqueue_style( ' socialfloating', plugin_dir_url( __FILE__ ) . 'css/socialfloating.css', array(), '0.1.0' );
	wp_enqueue_script( 'fontawesome', plugin_dir_url( __FILE__ ) . 'js/fontawsome.js', array(), '6.7.1' );
}
...
/**
 * Nightingale companion social widgets
 *
 * @return void
 */
					...
					foreach ( $socials as $social ) {
						...
						if ( get_theme_mod( 'social_' . $social . '_on' ) === 'yes' ) {
							...
							echo "{
								enabled: true,
								icon: '" . esc_html( $icon ) . "',
								name: '" . esc_html( $social ) . "',
								link: '" . esc_html( $link ) . "',
								title: 'Visit us on " . esc_html( $title ) . "',
								color: '#E8EDEE'
							},";
						}
					}
...
/**
 * Nightingale companion customize socials
 *
 * @param mixed $wp_customize customize class.
 * @return void
 */
function nightingale_companion_customize_socials( $wp_customize ) {
	...
	$wp_customize->add_control(
		'social_twitter_on',
		array(
			'label'           => esc_html__( 'Show X.com link?', 'nightingale' ),
			'section'         => 'section_social',
			'description'     => esc_html__( 'Do you wish to show a X.com link on your widget?', 'nightingale' ),
			'type'            => 'radio',
			'choices'         => array(
				'yes' => esc_html__( 'Yes', 'nightingale' ),
				'no'  => esc_html__( 'No', 'nightingale' ),
	...
	$wp_customize->add_control(
		'social_twitter_link',
		array(
			'label'           => esc_html__( 'Link to your X.com page (full url)', 'nightingale' ),
			'section'         => 'section_social',
			'type'            => 'text',
			'active_callback' => function () use ( $wp_customize ) {
