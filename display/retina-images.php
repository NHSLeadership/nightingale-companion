 ...
 */
add_action( 'wp_enqueue_scripts', 'nightingale_companion_retina_support_enqueue_scripts' );
/**
 * Enqueueing retina.js
 *
 * This function is attached to the 'wp_enqueue_scripts' action hook.
 */
function nightingale_companion_retina_support_enqueue_scripts() {
	wp_enqueue_script( 'retina_js', plugins_url( 'js/retina.js', __FILE__ ), '', '20200602', true );
}

add_filter( 'wp_generate_attachment_metadata', 'nightingale_companion_retina_support_attachment_meta', 10, 2 );
/**
 * Retina images
 *
 ...
 */
function nightingale_companion_retina_support_attachment_meta( $metadata, $attachment_id ) {
	foreach ( $metadata as $key => $value ) {
		if ( is_array( $value ) ) {
			foreach ( $value as $image => $attr ) {
				if ( is_array( $attr ) ) {
					nightingale_companion_retina_support_create_images( get_attached_file( $attachment_id ), $attr['width'], $attr['height'], true );
				}
			}
		}
	}
 ...
 */
function nightingale_companion_retina_support_create_images( $file, $width, $height, $crop = false ) {
	if ( $width || $height ) {
 ...
 */
function nightingale_companion_delete_retina_support_images( $attachment_id ) {
